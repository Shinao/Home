...
'custom_left_text' : "",
'custom_right_text' : "",
'email_notifications_addresses' : "rmonnerat.pro@gmail.com",
'email_notifications_enabled' : True,
'email_notifications_picture_time_span' : "4",
'email_notifications_smtp_account' : "malarm.dashboard@gmail.com",
'email_notifications_smtp_password' : "malarmdashboard",
'email_notifications_smtp_port' : "587",
'email_notifications_smtp_server' : "smtp.gmail.com",
'email_notifications_smtp_tls' : True,
'enabled' : True,
'event_gap' : "60",
'extra_options' : [],
'frame_change_threshold' : "1.0028645833333335",
'framerate' : "4",
'friday_from' : "",
'friday_to' : "",
'hue' : "50",
...
class ApiHandler(BaseHandler):
    @asynchronous
    def get(self, camera_id=None, api_key='', op=None):
	if api_key != 'XXXXX':
	    raise HTTPError('404', 'Wrong API KEY')
	...
	if op == 'enable':
	    ...
	    custom_ui_config['motion_detection'] = True
	    local_config = config.get_camera(camera_id)
	    local_config = config.motion_camera_ui_to_dict(custom_ui_config, local_config)
	    config.set_camera(camera_id, local_config)
		
            self.finish({'reload': False, 'reboot': True, 'error': None})
	    call(["reboot"])
	    ...
	    custom_ui_config['motion_detection'] = False
            local_config = config.get_camera(camera_id)
	    local_config = config.motion_camera_ui_to_dict(custom_ui_config, local_config)
	    config.set_camera(camera_id, local_config)
...
class ConfigHandler(BaseHandler):
    ...
    def get(self, camera_id=None, op=None):
	    ...
	    self.enable_motion(camera_id)

	elif op == 'disable':
	    self.disable_motion(camera_id)
    ...
    def backup(self):
        ...
        filename = 'motionpie-config.tar.gz'
...
class LogHandler(BaseHandler):
    LOGS = {
        ...
        'motioneye': ('/var/log/motioneye.log', 'motioneye.log'),
        'messages': ('/var/log/messages', 'messages.log'),
        'boot': ('/var/log/boot.log', 'boot.log'),
